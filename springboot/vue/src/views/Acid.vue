<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload action="http://localhost:9090/acid/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" border stripe header-cell-class-name="headerBg"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="idNum" label="身份证号" width="120"></el-table-column>
      <el-table-column prop="phone" label="学生电话" width="90"></el-table-column>
      <el-table-column prop="time" label="预约时间"></el-table-column>
      <el-table-column prop="place" label="预约地点" width="100"></el-table-column>
    ...
    <el-dialog title="学生预约申请信息" :visible.sync="dialogFormVisible" width="30%">
...
export default {
  ...
  methods: {
    load(){
      this.request.get("http://localhost:9090/acid/page",{
        params:{
    ...
    save(){
      this.request.post("/acid",this.form).then(res => {
        if(res){
    ...
    del(id){
      this.request.delete("/acid/" + id).then(res => {
        if(res){
    ...
    exp() {
      window.open("http://localhost:9090/acid/export")
    },
