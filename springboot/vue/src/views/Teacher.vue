<template>
  <div>
    ...
    <el-table :data="tableData" border stripe header-cell-class-name="headerBg"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="address" label="办公地址" width="100"></el-table-column>
      <el-table-column prop="location" label="家庭住址"></el-table-column>
  ...
  methods: {
    ...
    delBatch(){
      ...
      this.request.post("/teacher/del/batch",ids).then(res => {
        if(res){
          this.$message.success("批量删除教师信息成功！")
          this.load()
        }else{
          this.$message.error("批量删除教师信息失败！")
        }
      })
    },
    reset(){
