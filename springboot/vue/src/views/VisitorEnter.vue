<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload action="http://localhost:9090/visitorEnter/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" border stripe header-cell-class-name="headerBg"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="reason" label="入校缘由"></el-table-column>
      <el-table-column prop="place" label="来校地点"></el-table-column>
      ...
      <el-table-column label="健康码" width="100px">
        <img src="http://localhost:9090/file/660e3a24ced24791b63e8528a96fad8d.jpg" alt="" style="width: 90px;height: 90px">
      </el-table-column>
      <el-table-column label="行程码" width="100px">
        <img src="http://localhost:9090/file/73d45e596b7844509ee92ff23164a55b.jpg" alt="" style="width: 90px;height: 90px">
      </el-table-column>
      <el-table-column label="核酸结果" width="100px">
        <img src="http://localhost:9090/file/9d542589107946c18772eee3c4f1c31b.jpg" alt="" style="width: 90px;height: 90px">
      </el-table-column>
    ...
    <el-dialog title="访客入校申请" :visible.sync="dialogFormVisible" width="30%">
      <el-form label-width="100px" size="small">
        ...
        <el-form-item label="核酸结果">
          ...
          >
            <img v-if="form.acid" :src="form.acid" class="avatar">
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    load(){
      this.request.get("http://localhost:9090/visitorEnter/page",{
        params:{
    ...
    save(){
      this.request.post("/visitorEnter",this.form).then(res => {
        if(res){
    ...
    changeEnable(row) {
      this.request.post("/visitorEnter", row).then(res => {
        if (res) {
    ...
    del(id){
      this.request.delete("/visitorEnter/" + id).then(res => {
        if(res){
    ...
    delBatch(){
      ...
      this.request.post("/visitorEnter/del/batch",ids).then(res => {
        if(res){
    ...
    exp() {
      window.open("http://localhost:9090/visitorEnter/export")
    },
